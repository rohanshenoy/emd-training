...
class CNNEMD:
    ...
    def ittrain(self,num_filt, kernel_size, num_dens_neurons, num_dens_layers, num_conv_2d, num_epochs):
        ...
        current_directory=os.getcwd()
        final_directory=os.path.join(current_directory,r'convmodels')
        if not os.path.exists(final_directory):
            os.makedirs(final_directory)
        callbacks = [ModelCheckpoint('convmodels/emd_model_best.h5', monitor='val_loss', verbose=1, save_best_only=True),
                     ModelCheckpoint('convmodels/emd_model_last.h5', monitor='val_loss', verbose=1, save_last_only=True),
                    ]
